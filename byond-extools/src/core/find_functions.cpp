...
bool Core::find_functions()
{
	...
	FIND_OR_DIE(AddSleeperToList, "55 8B EC 83 EC 0C 8B ?? ?? 53 56 8B 35 ?? ?? ?? ?? 8B 80 ?? ?? ?? ?? 57 8B 3D ?? ?? ?? ?? 8B D7");
	FIND_OR_DIE(RunDM, "53 8B DC 83 EC 08 83 E4 F8 83 C4 04 55 8B 6B 04 89 6C 24 04 8B EC 6A FF 68 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 50 51 53 81 EC ?? ?? ?? ?? A1 ?? ?? ?? ?? 33 C5 89 45 E4 56 57 50 8D 45 F4");
	FIND_OR_DIE(TranslateProcNameToProcId, "55 8B EC 8B 4D 14 0F B6 C1 48 83 F8 53 0F 87 ?? ?? ?? ?? 0F B6 ?? ?? ?? ?? ?? FF 24 ?? ?? ?? ?? ?? FF 75 20 FF 75 10 FF 75 0C FF 75 08 FF 75 18 51 E8");
	FIND_OR_DIE(Output, "55 8B EC 81 EC ?? ?? ?? ?? A1 ?? ?? ?? ?? 33 C5 89 45 FC 8B 45 08 8B 4D 10 89 ?? ?? ?? ?? ?? 53 8B 5D 18 89 ?? ?? ?? ?? ?? 56 8B 75 14");
	FIND_OR_DIE(TextToPath, "55 8B EC 83 EC 08 56 57 8B 7D 08 57 E8 ?? ?? ?? ?? 83 C4 04 8B 30 A1 ?? ?? ?? ?? 89 45 FC A1 ?? ?? ?? ?? 89 45 F8 81 FF");
	FIND_OR_DIE(IsType, "55 8B EC 8B 4D 08 0F B6 C1 48 83 F8 53 77 22 0F B6 ?? ?? ?? ?? ?? FF 24 ?? ?? ?? ?? ?? FF 75 14 FF 75 10 FF 75 0C 51");
	FIND_OR_DIE(GetStep, "55 8B EC FF 75 10 FF 75 0C E8 ?? ?? ?? ?? 8B 4D 14 0F B6 C9 51 52 50 E8 ?? ?? ?? ?? 8B 4D 08 83 C4 14 89 01 8B C1 89 51 04 5D C2 10 00");
	FIND_OR_DIE(FormatString, "55 8B EC 83 EC 20 8B ?? ?? ?? ?? ?? 0F 57 C0 A1 ?? ?? ?? ?? 53 56 33 F6 89 4D F0 57 B9 ?? ?? ?? ?? 89 75 F4 89 45 FC F3 0F 11 45 EC C7 45 ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 7D 08 8B CF 8D 51 01 8A 01 41 84 C0 75 F9");
	switch(ByondVersion) {
		case 512:
	...
	char* dick = (char*)Pocket::Sigscan::FindPattern(BYONDCORE, "E8 ?? ?? ?? ?? 8B ?? ?? ?? ?? ?? 83 C4 10 8B 40 04 89 41 10 C7 41 ?? ?? ?? ?? ?? F6 47 04 08 8B 77 2C 89 75 F8 0F 84", 1);
	GetProcSetupEntry = (GetProcSetupEntryPtr)(dick + *(int*)dick + 4);
	...
	proc_setup_table_ptr = *(ProcSetupEntry * ***)Pocket::Sigscan::FindPattern(BYONDCORE, "A1 ?? ?? ?? ?? FF 34 B8 FF D6 47 83 C4 04 3B ?? ?? ?? ?? ?? 72 EA FF 35 ?? ?? ?? ?? FF D6 33 FF 83 C4 04 39 ?? ?? ?? ?? ?? 76 1E", 1);
	...
	sleeper_list = *(SleeperList**)Pocket::Sigscan::FindPattern(BYONDCORE, "8B 35 ?? ?? ?? ?? 8B 80 88 00 00 00 57 8B 3D ?? ?? ?? ?? 8B D7 89 45 FC A1 ?? ?? ?? ?? 8B D8 89 45 F8 89 75 F4 3B DA", 2);
#else
...
#warning TODO: GetAssocElement
#warning TODO: GetGlobalByName
#warning TODO: global_var_table
#warning TODO: IncRefCount
#warning TODO: DecRefCount
#warning TODO: x_ref_count_call
